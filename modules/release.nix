...
in
{
  ...
  config.build = rec {
    ...
    otaMetadata = (rec {
      ...
      arrowos = pkgs.writeText "arrowos-${config.device}.json" (
        # https://github.com/ArrowOS/android_packages_apps_Updater#server-requirements
        builtins.toJSON {
          response = [
            {
              "datetime" = config.buildDateTime;
    ...
    writeOtaMetadata = { otaFile, path }: {
      ...
      '';
      arrowos = ''
        sed -e "s:\"ROM_SIZE\":$(du -b ${otaFile} | cut -f1):" ${otaMetadata} > ${path}/arrowos-${config.device}.json
      '';
    }.${config.apps.updater.flavor};
