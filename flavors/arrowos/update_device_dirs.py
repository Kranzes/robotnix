...
def fetch_device_dirs(
    ...
    while len(dirs_to_fetch) > 0:
        ...
        arrow_dependencies_filename = os.path.join(
            dir_info["path"], "arrow.dependencies"
        )
        if os.path.exists(arrow_dependencies_filename):
            arrow_dependencies = json.load(open(arrow_dependencies_filename))

            for dep in arrow_dependencies:
                if dep["target_path"] not in dirs_fetched:
                    if dep.get("remote") == "gitlab":
                        dirs_to_fetch.add(
                            (
                                dep["target_path"],
                                f"https://gitlab.com/{dep['repository']}",
                            )
                        )
                    if dep.get("remote") == "github":
                        dirs_to_fetch.add(
                            (
                                dep["target_path"],
                                f"https://github.com/{dep['repository']}",
                            )
                        )
                else:
                    if dep.get("remote") == "github":
                ...
                dir_info["deps"] = [dep["target_path"] for dep in arrow_dependencies]
        else:
            dir_info["deps"] = []
...
def fetch_vendor_dirs(
    ...
    for vendor in required_vendor:
        ...
        url = f"{url_base}/android_{relpath.replace('/', '_')}_{device}"
...
def main() -> None:
    ...
    parser.add_argument("--branch", help="arrowos version", default="")
    parser.add_argument(
        "product",
        nargs="*",
    ...
    fetch_device_dirs(
        metadata,
        "https://github.com/ArrowOS-Devices",
        args.branch,
        device_dirs,
